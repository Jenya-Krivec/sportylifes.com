...
class Picture {
    ...
    add(id) {
        const row = '' +
            '<div class="relative">\n' +
            '    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 add-after hidden" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Image">\n' +
            ...
            '       <div class="flex justify-center items-center flex-col w-1/2 h-48 bg-white border border-green-600 rounded relative">\n' +
            '           <div class="flex justify-center items-center w-6 h-6 bg-green-600 text-sm text-white rounded-full hidden cursor-pointer absolute top-1 right-1" id="close-'+(id+1)+'" title="Complete">✓</div>\n' +
            '           <div>\n' +
            '               <label for="'+(id+1)+'-img-en" class="text-sm text-gray-500">Image for Desktop:</label>\n' +
            '               <input name="'+(id+1)+'-img-en" type="file" accept="image/*" class="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:bg-green-600 file:text-white hover:file:bg-green-600">\n' +
            '           </div>\n' +
            '           <div>\n' +
            '               <label for="'+(id+1)+'-img-en" class="text-sm text-gray-500">Image for Mobile:</label>\n' +
            '               <input name="'+(id+1)+'-img400-en" type="file" accept="image/*" class="mt-2 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:bg-green-600 file:text-white hover:file:bg-green-600">\n' +
            '           </div>\n' +
            '       </div>\n' +
            '    </div>\n' +
            '</div>';
