...
class Paragraph {
    ...
    add(id) {
        const row = '' +
            ...
            '    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute bottom-3 -left-4 add-after hidden" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Text">\n' +
            ...
            '    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="'+(id+1)+'" onChange="new Language().toggle({target: this})" title="Change language for Text">\n' +
