...
class Support {
    ...
    add(id) {
        let row = '' +
            ...
            '    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 add-after hidden" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Support">\n' +
        ...
        row += '     <input name="'+(id+1)+'-support-pt-0" class="text-base text-gray-700 text-justify indent-10 mb-4 w-full border-b-2 border-gray" type="text" value="">\n' +
            '    </div>\n' +
            '    <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-row-support" onclick="new Support().addRow({target: this})" data-id="'+(id+1)+'" title="Add Row">+</div>\n' +
            '    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="'+(id+1)+'" onChange="new Language().toggle({target: this})" title="Change language for Support">\n' +
