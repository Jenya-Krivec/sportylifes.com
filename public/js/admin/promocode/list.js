...
class List {
    ...
    add(id) {
        const row = '' +
            ...
            '    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after List">\n' +
            ...
            '        </ol>\n' +
            '    </div>\n' +
            '    <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-row-list" data-id="'+(id+1)+'" onclick="new List().addRow({target: this})" title="Add Row">+</div>\n' +
            '    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" onChange="new Language().toggle({target: this})" data-id="'+(id+1)+'" title="Change language for Caption">\n' +
