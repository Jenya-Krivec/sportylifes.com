...
class ProsCons {
    ...
    createRow(table, column, id, lang) {
        ...
        const classes = column === 'pros' ? 'text-green-600 bg-green-100' : 'text-gray-500 bg-gray-100';
        const child = column === 'pros' ? 0 : 1;
        const symbol =  column === 'pros' ? '+' : '-';
    ...
    add(id){
        const row = '' +
            ...
            '    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 add-after hidden" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Pros &amp; Cons">\n' +
            ...
            '                <div class="flex-row flex justify-start items-center text-green-600 bg-green-100 w-full h-6 rounded-sm mr-2 px-2">Pros</div>\n' +
            '            </div>\n' +
            '            <div class="flex-row flex justify-start items-center py-2 w-full">\n' +
            ...
            '            </div>\n' +
                        '<div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-pros" onclick="new ProsCons().addRow(`pros`, {target: this})" data-id="'+(id+1)+'" title="Add Pros">+</div>\n' +
            '        </div>\n' +
            ...
            '            </div>\n' +
            '            <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-cons" onclick="new ProsCons().addRow(`cons`, {target: this})" data-id="'+(id+1)+'" title="Add Cons">+</div>\n' +
            '        </div>\n' +
            '    </div>\n' +
            ...
            '        </div>\n' +
            '    </div>\n' +
            '    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="'+(id+1)+'" onChange="new Language().toggle({target: this})" title="Change language for Pros &amp; Cons">\n' +
