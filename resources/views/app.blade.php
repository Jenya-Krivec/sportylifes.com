    ...
    <nav class="flex items-center py-3 w-full">
        ...
        <div class="relative align-self-center ml-auto mr-2">
            <button class="block text-white focus:outline-none" id="language" aria-haspopup="true" aria-expanded="false">
                <span class="hover:bg-green-700 p-2 transition-all duration-500">
                    @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
                        @if($locale == app()->getLocale())
                            {{$language}}
                        @endif
            ...
            <div class="dropdown-menu absolute right-0 mt-2 py-2 w-48 bg-white border-2 border-green-600 rounded-md shadow-lg hidden z-50">
                @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
                    <a class="block px-4 py-2 text-black hover:text-white hover:bg-green-600 transition-all duration-500" href="{{\App\Helpers\Language::getUri($locale)}}">{{$language}}</a>
                @endforeach
            </div>
        </div>
    </nav>
...
@yield('content')
<button class="fixed bottom-20 right-4 bg-white rounded-full p-2 hidden border-2 border-green-600 z-20" id="go-up">
    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="rgb(5,150,105)">
...
<div class="fixed -bottom-40 left-0 right-0 bg-transparent z-20 transition-all duration-1000 hidden" id="cookie-message">
    <div class="bg-white w-full lg:w-1/3 mx-auto rounded-md p-6 shadow-lg border-2 border-green-600">
        ...
        <div class="flex justify-center mt-4">
            <button class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full transition-all duration-500" id="cookie-accept">@lang('app.Accept')</button>
