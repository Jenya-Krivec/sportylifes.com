...
@section('content')
    <main class="pt-16 bg-gray-100 font-serif">
        ...
        <!--Navigation -->
        <div class="w-full flex justify-center items-center py-4 text-sm sm:text-base text-center bg-gray-100 relative">
        ...
        <form method="POST" action="{{route('admin.storePromoCode', ['key' => $bookmaker['key']])}}" enctype="multipart/form-data" class="w-full bg-gray-100 px-0 lg:px-28 pt-4 form">
            ...
            <div class="w-full bg-gray-100 px-0 mt-4">
                <div class="px-2 sm:px-10 py-4 bg-white">
                    <span data-url="{{$bookmaker['url']}}" class="bg-green-600 text-xs sm:text-lg border-2 border-green-600 text-white rounded-md hover:bg-white hover:text-black flex justify-center items-center cursor-pointer transition-all w-44 duration-500 my-2 mx-auto px-16 py-2">@lang('promocode.Registration')</span>
                </div>
            </div>
        ...
        <div id="add-section" class="fixed border-2 border-green-600 bg-white rounded-md flex justify-center items-center -left-52 p-2 flex-col bottom-1/2 transition-all duration-500 z-30" style="transform: translateY(50%);">
    ...
    <script defer type="text/javascript" src="{{ asset('js/promocode/rate.js').'?v='.filemtime('js/promo/rate.js') }}"></script>
