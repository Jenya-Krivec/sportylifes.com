...
@section('content')
    <main class="pt-24">
        ...
        <form action="{{ route('admin.storeCurrencies') }}" method="POST" enctype="multipart/form-data" class="flex justify-center items-center flex-col form">
            ...
            <span class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-currency hover:bg-white hover:text-black border-2 border-green-600" data-id="{{$currency['id']}}" title="Add Currency">+</span>
        </form>
    </main>
