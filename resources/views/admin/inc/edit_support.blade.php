<div class="relative border-b-2 border-gray-200">
    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Support">
    ...
    @endforeach
    <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-row-support" data-id="{{$id}}" title="Add Row">+</div>
    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="{{$id}}" title="Change language for Support">
