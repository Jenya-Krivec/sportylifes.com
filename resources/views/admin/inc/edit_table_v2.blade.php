<div class="relative">
    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Сomparison">
    ...
    <div data-name="{{$id.'-'.$lang}}" class="flex flex-col justify-center items-center border-gray-200 border-2 rounded-xl shadow-gray-400 shadow-lg @if($lang !== 'en') hidden @endif">
        ...
        <div class="flex justify-end items-center w-full rounded-br-xl rounded-bl-xl {{ $isEven ? 'bg-gray-100' : '' }}">
            ...
            <span class="px-1 py-2 sm:p-2 w-28 flex-shrink-0 text-center">
                <a href="" class="text-black text-xs link-for-review-2">{{trans('review.Review', ['bookmaker' => json_decode($value, true)[0][2]])}}</a>
            </span>
            <span class="px-1 py-2 sm:p-2 w-28 flex-shrink-0 text-center">
                <a href="" class="text-black text-xs link-for-review-3">{{trans('review.Review', ['bookmaker' => json_decode($value, true)[0][3]])}}</a>
            </span>
        </div>
        <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 my-1 cursor-pointer add-row-comparison" data-id="{{$id}}" title="Add Row">+</div>
    </div>
    @endforeach
    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="{{$id}}" title="Change language for Сomparison">
