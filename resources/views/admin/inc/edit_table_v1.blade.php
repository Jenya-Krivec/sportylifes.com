<div class="relative border-b-2 border-gray-200">
    <div class="flex items-center text-white bg-green-600 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Pros & Cons">
    ...
    <div data-name="{{$id.'-'.$lang}}" class="flex flex-col sm:flex-row justify-center items-start @if($lang !== 'en') hidden @endif">
        <div class="w-full sm:w-1/2 flex-col justify-center items-start flex mr-2 pros">
            ...
            @endif
            <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-pros" data-id="{{$id}}" title="Add Pros">+</div>
        </div>
        <div class="w-full sm:w-1/2 flex-col justify-center items-start flex cons">
            ...
            @endif
            <div class="flex items-center justify-center self-center text-white bg-green-600 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-cons" data-id="{{$id}}" title="Add Cons">+</div>
        </div>
    </div>
    @endforeach
    <select class="absolute top-1 -right-11 border-2 border-green-600 text-xs rounded language" data-id="{{$id}}" title="Change language for Pros & Cons">
